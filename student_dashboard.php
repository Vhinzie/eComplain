...
// Ensure $action is always defined to avoid undefined variable warnings
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'submit_complaint') {
        $assigned_role = isset($_POST['assigned_role']) ? trim($_POST['assigned_role']) : '';
...
<head>
    ...
    <title>Student Dashboard - ACLC eComplain</title>
...
<body>
    <header class="header">
        <h1>ACLC eComplain - Student Dashboard</h1>
    </header>
                        ...
                        <td>
                            <?php if ($complaint['status'] === 'pending'): ?>
                                <form method="post" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" onsubmit="return confirm('Are you sure you want to cancel this complaint?');" style="margin:0">
                                    ...
                                    <input type="hidden" name="complaint_id" value="<?= intval($complaint['id']) ?>">
                                    <button type="submit" class="btn-small">Cancel</button>
                                </form>
                            <?php elseif ($complaint['status'] === 'cancelled'): ?>
                                <?php $can_undo = (time() - strtotime($complaint['updated_at'])) <= (UNDO_WINDOW_MINUTES * 60); ?>
                                <?php if ($can_undo): ?>
